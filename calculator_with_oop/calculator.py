...
class Calculator(object):
    ...
    def calculate(self):
        ...
        try:
            if "%" in expression:
                ...
                self.log_operation('percentage', expression, result)
            elif "!" in expression:
                result = str(math.factorial(int(self.factorial_expression)))
                self.log_operation('factorial', self.factorial_expression, result)
            elif "^" in expression:
                parts = expression.split("^")
                result = str(eval(parts[0]) ** eval(parts[1]))
                self.log_operation('to_power', parts[0] + "^" + parts[1], result)
            else:
    ...
    def show_statistics(self):
        ...
        total_additions = self.db.cursor.execute(
            "SELECT COUNT(*) FROM operations WHERE operation = 'calculation' AND operands LIKE '%+%'").fetchone()[0]
        ...
        # Other operations
        total_to_power = \
            self.db.cursor.execute("SELECT COUNT(*) FROM operations WHERE operation = 'to_power'").fetchone()[0]
        total_percentage = \
